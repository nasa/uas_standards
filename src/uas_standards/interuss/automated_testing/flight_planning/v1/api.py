"""Data types and operations from Flight Planning Automated Testing Interface 0.7.0 OpenAPI"""
...
class OperatorRegistrationNumber(ImplicitDict):
    """Number provided by CAA or authorized representative for registering, licensing and/or identifying a remote pilot or operator."""
...
class AltitudeReference(str, Enum):
    """A code indicating the reference for a vertical distance. See AIXM 5.1 and FIXM 4.2.0 for W84 and SFC."""
...
class BasicFlightPlanInformation(ImplicitDict):
    ...
    uas_state: FunctionalState | None
    """State of the user's UAS associated with this flight plan as reported by the operator.  If the UAS reports its own state, that report will be reflected in telemetry.  If the operator reports the UAS state, that report will be reflected here.
    If a system accepts UAS state reports from both the operator and UAS, it is possible the reported state may differ between those two sources.
    """
...
class UpsertFlightPlanResponse(ImplicitDict):
    ...
    as_planned: FlightPlan | None
    """The flight plan information, as it was actually planned in the system.  When the USS modifies any of the requested information (e.g., changing serial number, updating telemetry, or not injecting telemetry), this field should contain all those modifications.  If this field is not populated, the USS is indicating the requested flight information was injected with no modifications."""
